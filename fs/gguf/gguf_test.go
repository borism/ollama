...
func createBinFile(tb testing.TB) string {
	...
	for i := range 8 {
		tensors = append(tensors, &gguftest.Tensor{
...
func TestOpenRejectsInvalidMagic(t *testing.T) {
	...
	if f != nil {
		f.Close()
		t.Fatal("gguf.Open returned a file for invalid magic")
	}
	if !errors.Is(err, gguf.ErrUnsupported) {
		t.Fatalf("gguf.Open error = %v, want ErrUnsupported", err)
	}
}
