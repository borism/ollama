...
status` shows every cluster setting and where it comes from. The other
...
- **Share the \<GPU name\> GPU**, e.g. "Share the Apple M2 Max GPU" --
  `OLLAMA_CLUSTER_SHARE` (disabled while cluster mode is off). It reads
  "Share this computer's GPU" on a machine without one, or before the
  server has started.
...
On macOS, the menu-bar menu has **Cluster Mode** and **Share the \<GPU
name\> GPU** items, each with a green (on) or red (off) dot; click one
to switch it. A **Placement** submenu below them picks the placement
policy. macOS asks once
